...
public class BETA_SETTINGS{
    ...
    private void FixedUpdate()
    {
        ...
        Vector3 dMove = (transform.forward * inputXY.y * speed + transform.right * inputXY.x * walkSpeedInternal) + Vector3.up * fps_Rigidbody.velocity.y;

        FixedUpdateCall(dMove);
    }

    public void ForceMoveFixedUpdate(Vector2 direction, bool crouching)
    {
        if (!forcedMovement) return;

        Vector3 dMove = transform.forward * direction.y * speed + transform.right * direction.x * speed;
    ...
    private void FixedUpdateCall(Vector3 dMove)
    {
        speed = walkSpeed;
